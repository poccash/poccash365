...
public final class Db {
  ...
  private static final ThreadLocal<Map<String, Map<Pc365Key, Object>>> transactionCaches = new ThreadLocal<>();
  private static final ThreadLocal<Map<String, Map<Pc365Key, Object>>> transactionBatches = new ThreadLocal<>();
  ...
  public static void shutdown() {
    if (dialect == SQLDialect.H2) {
      try ( Connection con = cp.getConnection(); Statement stmt = con.createStatement() ) {
        ...
        if(Pc365.getPropertyService().getBoolean(Props.DB_H2_DEFRAG_ON_SHUTDOWN)) {
  ...
  static <V> Map<Pc365Key, V> getCache(String tableName) {
    if (!isInTransaction()) {
      throw new IllegalStateException("Not in transaction");
    }
    //noinspection unchecked
    return (Map<Pc365Key, V>) transactionCaches.get().computeIfAbsent(tableName, k -> new HashMap<>());
  }

  static <V> Map<Pc365Key, V> getBatch(String tableName) {
    if (!isInTransaction()) {
      throw new IllegalStateException("Not in transaction");
    }
    //noinspection unchecked
    return (Map<Pc365Key, V>) transactionBatches.get().computeIfAbsent(tableName, k -> new HashMap<>());
  }
