...
import core.db.store.EscrowStore;
...
import static core.schema.Tables.ESCROW;
import static core.schema.Tables.ESCROW_DECISION;

public class SqlEscrowStore implements EscrowStore {
  private final Pc365Key.LongKeyFactory<Escrow> escrowDbKeyFactory = new DbKey.LongKeyFactory<Escrow>(ESCROW.ID) {
      @Override
      public Pc365Key newKey(Escrow escrow) {
        return escrow.dbKey;
      }
    };
  ...
  private final DbKey.LinkKeyFactory<Escrow.Decision> decisionDbKeyFactory =
      new DbKey.LinkKeyFactory<Escrow.Decision>("escrow_id", "account_id") {
        @Override
        public Pc365Key newKey(Escrow.Decision decision) {
  ...
  public SqlEscrowStore(DerivedTableManager derivedTableManager) {
    escrowTable = new VersionedEntitySqlTable<Escrow>("escrow", core.schema.Tables.ESCROW, escrowDbKeyFactory, derivedTableManager) {
      @Override
    ...
    decisionTable = new VersionedEntitySqlTable<Escrow.Decision>("escrow_decision", core.schema.Tables.ESCROW_DECISION, decisionDbKeyFactory, derivedTableManager) {
      @Override
  ...
  private void saveDecision(DSLContext ctx, Escrow.Decision decision) {
    ctx.mergeInto(ESCROW_DECISION, ESCROW_DECISION.ESCROW_ID, ESCROW_DECISION.ACCOUNT_ID, ESCROW_DECISION.DECISION, ESCROW_DECISION.HEIGHT, ESCROW_DECISION.LATEST)
            ...
            .values(decision.escrowId, decision.accountId, (int) Escrow.decisionToByte(decision.getDecision()), Pc365.getBlockchain().getHeight(), true)
            .execute();
  }

  @Override
  public Pc365Key.LongKeyFactory<Escrow> getEscrowDbKeyFactory() {
  ...
  private void saveEscrow(DSLContext ctx, Escrow escrow) {
    ctx.mergeInto(ESCROW, ESCROW.ID, ESCROW.SENDER_ID, ESCROW.RECIPIENT_ID, ESCROW.AMOUNT, ESCROW.REQUIRED_SIGNERS, ESCROW.DEADLINE, ESCROW.DEADLINE_ACTION, ESCROW.HEIGHT, ESCROW.LATEST)
            ...
            .values(escrow.id, escrow.senderId, escrow.recipientId, escrow.amountNQT, escrow.requiredSigners, escrow.deadline, (int) Escrow.decisionToByte(escrow.deadlineAction), Pc365.getBlockchain().getHeight(), true)
            .execute();
  }
