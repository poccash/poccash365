...
import core.db.store.DigitalGoodsStoreStore;
...
public class SqlDigitalGoodsStoreStore implements DigitalGoodsStoreStore {
  ...
  private static final DbKey.LongKeyFactory<DigitalGoodsStore.Purchase> feedbackDbKeyFactory
    = new DbKey.LongKeyFactory<DigitalGoodsStore.Purchase>(PURCHASE.ID) {
        @Override
        public Pc365Key newKey(DigitalGoodsStore.Purchase purchase) {
          return purchase.dbKey;
        }
      };

  private final Pc365Key.LongKeyFactory<DigitalGoodsStore.Purchase> purchaseDbKeyFactory
    = new DbKey.LongKeyFactory<DigitalGoodsStore.Purchase>(PURCHASE.ID) {
        @Override
        public Pc365Key newKey(DigitalGoodsStore.Purchase purchase) {
          return purchase.dbKey;
  ...
  private final Pc365Key.LongKeyFactory<DigitalGoodsStore.Goods> goodsDbKeyFactory = new DbKey.LongKeyFactory<DigitalGoodsStore.Goods>(GOODS.ID) {
      @Override
      public Pc365Key newKey(DigitalGoodsStore.Goods goods) {
        return goods.dbKey;
      }
    };
  ...
  public SqlDigitalGoodsStoreStore(DerivedTableManager derivedTableManager) {
    purchaseTable = new VersionedEntitySqlTable<DigitalGoodsStore.Purchase>("purchase", core.schema.Tables.PURCHASE, purchaseDbKeyFactory, derivedTableManager) {
      @Override
    ...
    feedbackTable = new VersionedValuesSqlTable<DigitalGoodsStore.Purchase, EncryptedData>("purchase_feedback", core.schema.Tables.PURCHASE_FEEDBACK, feedbackDbKeyFactory, derivedTableManager) {
      ...
      protected void save(DSLContext ctx, DigitalGoodsStore.Purchase purchase, EncryptedData encryptedData) {
        ...
        ctx.insertInto(
            ...
            purchase.getId(),
            data, nonce,
            core.Pc365.getBlockchain().getHeight(), true
        ).execute();
      }
    };

    publicFeedbackTable
        = new VersionedValuesSqlTable<DigitalGoodsStore.Purchase, String>("purchase_public_feedback", core.schema.Tables.PURCHASE_PUBLIC_FEEDBACK, publicFeedbackDbKeyFactory, derivedTableManager) {
      ...
      protected void save(DSLContext ctx, DigitalGoodsStore.Purchase purchase, String publicFeedback) {
        ctx.mergeInto(PURCHASE_PUBLIC_FEEDBACK, PURCHASE_PUBLIC_FEEDBACK.ID, PURCHASE_PUBLIC_FEEDBACK.PUBLIC_FEEDBACK, PURCHASE_PUBLIC_FEEDBACK.HEIGHT, PURCHASE_PUBLIC_FEEDBACK.LATEST)
                ...
                .values(purchase.getId(), publicFeedback, Pc365.getBlockchain().getHeight(), true)
                .execute();
      }
    };

    goodsTable = new VersionedEntitySqlTable<DigitalGoodsStore.Goods>("goods", core.schema.Tables.GOODS, goodsDbKeyFactory, derivedTableManager) {
      ...
      protected List<SortField<?>> defaultSort() {
        List<SortField<?>> sort = new ArrayList<>();
        sort.add(core.schema.Tables.GOODS.field("timestamp", Integer.class).desc());
        sort.add(core.schema.Tables.GOODS.field("id", Long.class).asc());
        return sort;
      }
    };
  }
  ...
  @Override
  public Pc365Key.LongKeyFactory<DigitalGoodsStore.Purchase> getFeedbackDbKeyFactory() {
    return feedbackDbKeyFactory;
  }

  @Override
  public Pc365Key.LongKeyFactory<DigitalGoodsStore.Purchase> getPurchaseDbKeyFactory() {
  ...
  @Override
  public Pc365Key.LongKeyFactory<DigitalGoodsStore.Goods> getGoodsDbKeyFactory() {
  ...
  private void saveGoods(DSLContext ctx, DigitalGoodsStore.Goods goods) {
    ctx.mergeInto(GOODS, GOODS.ID, GOODS.SELLER_ID, GOODS.NAME, GOODS.DESCRIPTION, GOODS.TAGS, GOODS.TIMESTAMP, GOODS.QUANTITY, GOODS.PRICE, GOODS.DELISTED, GOODS.HEIGHT, GOODS.LATEST)
            ...
            .values(goods.getId(), goods.getSellerId(), goods.getName(), goods.getDescription(), goods.getTags(), goods.getTimestamp(), goods.getQuantity(), goods.getPriceNQT(), goods.isDelisted(), Pc365.getBlockchain().getHeight(), true)
            .execute();
  }
