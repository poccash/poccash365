...
public abstract class EntitySqlTable<T> extends DerivedSqlTable implements EntityTable<T> {
  ...
  EntitySqlTable(String table, TableImpl<?> tableClass, Pc365Key.Factory<T> dbKeyFactory, DerivedTableManager derivedTableManager) {
    this(table, tableClass, dbKeyFactory, false, derivedTableManager);
  }

  EntitySqlTable(String table, TableImpl<?> tableClass, Pc365Key.Factory<T> dbKeyFactory, boolean multiversion, DerivedTableManager derivedTableManager) {
  ...
  private Map<Pc365Key, T> getCache() {
  ...
  public final void checkAvailable(int height) {
    if (multiversion && height < Pc365.getBlockchainProcessor().getMinRollbackHeight()) {
      throw new IllegalArgumentException("Historical data as of height " + height + " not available, set core.trimDerivedTables=false and re-scan");
    }
  }

  @Override
  public T get(Pc365Key nxtKey) {
  ...
  @Override
  public T get(Pc365Key nxtKey, int height) {
    DbKey dbKey = (DbKey) nxtKey;
