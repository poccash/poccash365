...
public abstract class VersionedBatchEntitySqlTable<T> extends VersionedEntitySqlTable<T> implements VersionedBatchEntityTable<T> {
  ...
  public void insert(T t) {
    assertNotInTransaction();
    Pc365Key key = dbKeyFactory.newKey(t);
    getBatch().put(key, t);
    getCache().put(key, t);
  }
  ...
  public void finish() {
    assertNotInTransaction();
    Set<Pc365Key> keySet = getBatch().keySet();
    if (keySet.isEmpty()) {
      return;
    }
  ...
  @Override
  public T get(Pc365Key dbKey, int height) {
    assertInTransaction();
  ...
  @Override
  public Map<Pc365Key, T> getBatch() {
    return Db.getBatch(table);
  }

  @Override
  public Cache<Pc365Key, T> getCache() {
