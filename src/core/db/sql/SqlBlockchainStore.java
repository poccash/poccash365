...
import core.db.store.IndirectIncomingStore;
...
import static core.schema.Tables.TRANSACTION;

public class SqlBlockchainStore implements BlockchainStore {

  private final TransactionDb transactionDb = Pc365.getDbs().getTransactionDb();
  private final BlockDb blockDb = Pc365.getDbs().getBlockDb();
  ...
  public Collection<Block> getBlocks(int from, int to) {
    return Db.useDSLContext(ctx -> {
      int blockchainHeight = Pc365.getBlockchain().getHeight();
      return
  ...
  public Collection<Transaction> getTransactions(Account account, int numberOfConfirmations, byte type, byte subtype, int blockTimestamp, int from, int to, boolean includeIndirectIncoming) {
    int height = numberOfConfirmations > 0 ? Pc365.getBlockchain().getHeight() - numberOfConfirmations : Integer.MAX_VALUE;
    if (height < 0) {
      throw new IllegalArgumentException("Number of confirmations required " + numberOfConfirmations + " exceeds current blockchain height " + Pc365.getBlockchain().getHeight());
    }
    return Db.useDSLContext(ctx -> {
