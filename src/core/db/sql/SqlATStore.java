...
import core.schema.tables.records.AtRecord;
import core.schema.tables.records.AtStateRecord;
import org.jooq.*;
...
import static core.schema.Tables.*;

public class SqlATStore implements ATStore {

  private final Pc365Key.LongKeyFactory<core.at.AT> atDbKeyFactory = new DbKey.LongKeyFactory<core.at.AT>(AT.ID) {
      @Override
      public Pc365Key newKey(core.at.AT at) {
        return at.dbKey;
      }
    };
  private final VersionedEntityTable<core.at.AT> atTable;

  private final Pc365Key.LongKeyFactory<core.at.AT.ATState> atStateDbKeyFactory = new DbKey.LongKeyFactory<core.at.AT.ATState>(AT_STATE.AT_ID) {
      @Override
      public Pc365Key newKey(core.at.AT.ATState atState) {
        return atState.dbKey;
      }
    };

  private final VersionedEntityTable<core.at.AT.ATState> atStateTable;

  public SqlATStore(DerivedTableManager derivedTableManager) {
    atTable = new VersionedEntitySqlTable<core.at.AT>("at", core.schema.Tables.AT, atDbKeyFactory, derivedTableManager) {
      @Override
      protected core.at.AT load(DSLContext ctx, Record rs) {
        throw new RuntimeException("AT attempted to be created with atTable.load");
      }

      @Override
      protected void save(DSLContext ctx, core.at.AT at) {
        saveAT(ctx, at);
      }
    ...
    atStateTable = new VersionedEntitySqlTable<core.at.AT.ATState>("at_state", core.schema.Tables.AT_STATE, atStateDbKeyFactory, derivedTableManager) {
      @Override
      protected core.at.AT.ATState load(DSLContext ctx, Record rs) {
        return new SqlATState(rs);
      }

      @Override
      protected void save(DSLContext ctx, core.at.AT.ATState atState) {
  ...
  private void saveATState(DSLContext ctx, core.at.AT.ATState atState) {
    ctx.mergeInto(AT_STATE, AT_STATE.AT_ID, AT_STATE.STATE, AT_STATE.PREV_HEIGHT, AT_STATE.NEXT_HEIGHT, AT_STATE.SLEEP_BETWEEN, AT_STATE.PREV_BALANCE, AT_STATE.FREEZE_WHEN_SAME_BALANCE, AT_STATE.MIN_ACTIVATE_AMOUNT, AT_STATE.HEIGHT, AT_STATE.LATEST)
            ...
            .values(atState.getATId(), core.at.AT.compressState(atState.getState()), atState.getPrevHeight(), atState.getNextHeight(), atState.getSleepBetween(), atState.getPrevBalance(), atState.getFreezeWhenSameBalance(), atState.getMinActivationAmount(), Pc365.getBlockchain().getHeight(), true)
            .execute();
  }

  private void saveAT(DSLContext ctx, core.at.AT at) {
    ctx.insertInto(
      AT,
      ...
      core.at.AT.compressState(at.getApCodeBytes()), Pc365.getBlockchain().getHeight()
    ).execute();
  }
  ...
  public List<Long> getOrderedATs() {
    return Db.useDSLContext(ctx -> {
      return ctx.selectFrom(
              ...
              AT_STATE.NEXT_HEIGHT.lessOrEqual(Pc365.getBlockchain().getHeight() + 1)
      ).and(
              ACCOUNT.BALANCE.greaterOrEqual(
                      AtConstants.getInstance().stepFee(Pc365.getBlockchain().getHeight())
                              * AtConstants.getInstance().apiStepMultiplier(Pc365.getBlockchain().getHeight())
              )
      ).and(
  ...
  @Override
  public core.at.AT getAT(Long id) {
    return Db.useDSLContext(ctx -> {
  ...
  private core.at.AT createAT(AtRecord at, AtStateRecord atState) {
    return new AT(AtApiHelper.getByteArray(at.getId()), AtApiHelper.getByteArray(at.getCreatorId()), at.getName(), at.getDescription(), at.getVersion(),
            core.at.AT.decompressState(atState.getState()), at.getCsize(), at.getDsize(), at.getCUserStackBytes(), at.getCCallStackBytes(), at.getCreationHeight(), atState.getSleepBetween(), atState.getNextHeight(),
            atState.getFreezeWhenSameBalance(), atState.getMinActivateAmount(), core.at.AT.decompressState(at.getApCode()));
  }
  ...
  @Override
  public Pc365Key.LongKeyFactory<core.at.AT> getAtDbKeyFactory() {
    return atDbKeyFactory;
  }

  @Override
  public VersionedEntityTable<core.at.AT> getAtTable() {
    return atTable;
  }

  @Override
  public Pc365Key.LongKeyFactory<core.at.AT.ATState> getAtStateDbKeyFactory() {
    return atStateDbKeyFactory;
  }

  @Override
  public VersionedEntityTable<core.at.AT.ATState> getAtStateTable() {
    return atStateTable;
  }
  ...
  class SqlATState extends core.at.AT.ATState {
