...
import static core.Constants.MIN_VERSION;
import static core.peer.Peer.isHigherOrEqualVersion;
import static core.props.Props.P2P_ENABLE_TX_REBROADCAST;
import static core.props.Props.P2P_SEND_TO_LIMIT;
import static core.util.JSON.prepareRequest;
...
public final class Peers {
  ...
  public static boolean isSupportedUserAgent(String header) {
    if(header == null || header.isEmpty() || ! header.trim().startsWith("CORE/")) {
      return false;
    } else {
      try {
        return isHigherOrEqualVersion(MIN_VERSION, Version.parse(header.trim().substring("CORE/".length())));
  ...
  static final int DEFAULT_PEER_PORT = 7108;
  ...
  public static void init(TimeService timeService, AccountService accountService, Blockchain blockchain, TransactionProcessor transactionProcessor,
    ...
    if (myAddress != null && myAddress.endsWith(":" + TESTNET_PEER_PORT) && !Pc365.getPropertyService().getBoolean(Props.DEV_TESTNET)) {
      throw new RuntimeException("Port " + TESTNET_PEER_PORT + " should only be used for testnet!!!");
    }
    myPeerServerPort = propertyService.getInt(Props.P2P_PORT);
    if (myPeerServerPort == TESTNET_PEER_PORT && !Pc365.getPropertyService().getBoolean(Props.DEV_TESTNET)) {
      throw new RuntimeException("Port " + TESTNET_PEER_PORT + " should only be used for testnet!!!");
    }
    useUpnp = propertyService.getBoolean(Props.P2P_UPNP);
    shareMyAddress = propertyService.getBoolean(Props.P2P_SHARE_MY_ADDRESS) && ! Pc365.getPropertyService().getBoolean(Props.DEV_OFFLINE);
    ...
    json.addProperty("application",  Pc365.APPLICATION);
    json.addProperty("version",      Pc365.VERSION.toString());
    ...
    if(propertyService.getBoolean(P2P_ENABLE_TX_REBROADCAST)) {
      rebroadcastPeers = Collections
              .unmodifiableSet(new HashSet<>(propertyService.getStringList(Pc365.getPropertyService().getBoolean(Props.DEV_TESTNET) ? Props.DEV_P2P_REBROADCAST_TO : Props.P2P_REBROADCAST_TO)));
    } else {
      rebroadcastPeers = Collections.emptySet();
    }

    List<String> wellKnownPeersList = propertyService.getStringList(Pc365.getPropertyService().getBoolean(Props.DEV_TESTNET) ? Props.DEV_P2P_BOOTSTRAP_PEERS : Props.P2P_BOOTSTRAP_PEERS);
    ...
    if (wellKnownPeersList.isEmpty() || Pc365.getPropertyService().getBoolean(Props.DEV_OFFLINE)) {
    ...
    communicationLoggingMask = propertyService.getInt(Props.CORE_COMMUNICATION_LOGGING_MASK);
    sendToPeersLimit = propertyService.getInt(P2P_SEND_TO_LIMIT);
    usePeersDb       = propertyService.getBoolean(Props.P2P_USE_PEERS_DB) && ! Pc365.getPropertyService().getBoolean(Props.DEV_OFFLINE);
    ...
    threadPool.runBeforeStart(new Runnable() {
      ...
      public void run() {
        ...
        if (usePeersDb) {
          ...
          loadPeers(Pc365.getDbs().getPeerDb().loadPeers());
        }
    ...
    if (! Pc365.getPropertyService().getBoolean(Props.DEV_OFFLINE)) {
  ...
  private static class Init {
    ...
    static void init(TimeService timeService, AccountService accountService, Blockchain blockchain, TransactionProcessor transactionProcessor,
      ...
      if (Peers.shareMyAddress) {
            ...
            if (gateway != null) {
              ...
              try {
                ...
                if (gateway.getSpecificPortMappingEntry(port, "TCP", portMapping)) {
                  ...
                  if (gateway.addPortMapping(port, port, localAddress.getHostAddress(), "TCP", "poccash365")) {
        ...
        port = Pc365.getPropertyService().getBoolean(Props.DEV_TESTNET) ? TESTNET_PEER_PORT : Peers.myPeerServerPort;
  ...
  private static final Runnable peerConnectingThread = new Runnable() {
    ...
    private void updateSavedPeers() {
      Set<String> oldPeers = new HashSet<>(Pc365.getDbs().getPeerDb().loadPeers());
      ...
      try {
        Pc365.getStores().beginTransaction();
        Pc365.getDbs().getPeerDb().deletePeers(toDelete);
        currentPeers.removeAll(oldPeers);
        Pc365.getDbs().getPeerDb().addPeers(currentPeers);
        Pc365.getStores().commitTransaction();
      } catch (Exception e) {
        Pc365.getStores().rollbackTransaction();
        throw e;
      } finally {
        Pc365.getStores().endTransaction();
      }
    }
  ...
  static Peer addPeer(final String address, final String announcedAddress) {
    ...
    if (Pc365.getPropertyService().getBoolean(Props.DEV_TESTNET) && peer.getPort() > 0 && peer.getPort() != TESTNET_PEER_PORT) {
  ...
  private static boolean peerEligibleForSending(Peer peer, boolean sendSameCOREclass) {
    return peer.isHigherOrEqualVersionThan(MIN_VERSION)
            && (! sendSameCOREclass || peer.isAtLeastMyVersion())
            && ! peer.isBlacklisted()
  ...
  public static Peer getAnyPeer(Peer.State state) {
    if(!connectWellKnownFinished) {
      ...
      if (wellKnownConnected >= connectWellKnownFirst) {
        ...
        logger.info("You can open your Pc365 Wallet in your favorite browser with: http://127.0.0.1:7107 or http://localhost:7107");
      }
    }
