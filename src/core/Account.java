...
public class Account {
  ...
  public static class AccountAsset {
    ...
    public final Pc365Key pc365Key;
    private long quantityQNT;
    private long unconfirmedQuantityQNT;

    protected AccountAsset(long accountId, long assetId, long quantityQNT, long unconfirmedQuantityQNT, Pc365Key pc365Key) {
      this.accountId = accountId;
      this.assetId = assetId;
      this.quantityQNT = quantityQNT;
      this.unconfirmedQuantityQNT = unconfirmedQuantityQNT;
      this.pc365Key = pc365Key;
    }

    public AccountAsset(Pc365Key pc365Key, long accountId, long assetId, long quantityQNT, long unconfirmedQuantityQNT) {
      this.accountId = accountId;
      this.assetId = assetId;
      this.pc365Key = pc365Key;
      this.quantityQNT = quantityQNT;
  ...
  public static class RewardRecipientAssignment {
    ...
    public RewardRecipientAssignment(Long accountId, Long prevRecipientId, Long recipientId, int fromHeight, Pc365Key pc365Key) {
      this.accountId = accountId;
  ...
  private static Pc365Key.LongKeyFactory<Account> accountPc365KeyFactory() {
    return Pc365.getStores().getAccountStore().getAccountKeyFactory();
  }

  private static VersionedBatchEntityTable<Account> accountTable() {
    return Pc365.getStores().getAccountStore().getAccountTable();
  }
  ...
  public Account(long id) {
    ...
    this.id = id;
    this.nxtKey = accountPc365KeyFactory().newKey(this.id);
    this.creationHeight = Pc365.getBlockchain().getHeight();
  }

  protected Account(long id, Pc365Key pc365Key, int creationHeight) {
    if (id != Crypto.rsDecode(Crypto.rsEncode(id))) {
      logger.log(Level.INFO, "CRITICAL ERROR: Reed-Solomon encoding fails for {0}", id);
    }
    this.id = id;
    this.nxtKey = pc365Key;
  ...
  public boolean setOrVerify(byte[] key, int height) {
    return Pc365.getStores().getAccountStore().setOrVerify(this, key, height);
  }
