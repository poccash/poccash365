...
public class Pc365Launcher {
    public static void main(String[] args) {
        Logger logger = LoggerFactory.getLogger(Pc365Launcher.class);
        ...
        if (canRunGui) {
            try {
                Class.forName("core.Pc365GUI")
                        .getDeclaredMethod("main", String[].class)
                        .invoke(null, (Object) args);
            } catch (ClassNotFoundException | NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
                logger.warn("Your build does not seem to include the Pc365GUI extension or it cannot be run. Running as headless...");
                Pc365.main(args);
            }
        } else {
            Pc365.main(args);
