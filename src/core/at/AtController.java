...
public abstract class AtController {
    ...
    private static final Logger debugLogger = Pc365.getPropertyService().getBoolean(Props.ENABLE_AT_DEBUG_LOG) ? logger : NOPLogger.NOP_LOGGER;
    ...
    private static int runSteps(AtMachineState state) {
        ...
        AtMachineProcessor processor = new AtMachineProcessor(state, Pc365.getPropertyService().getBoolean(Props.ENABLE_AT_DEBUG_LOG));
    ...
    private static void listCode(AtMachineState state, boolean disassembly, boolean determineJumps) {
        ...
        AtMachineProcessor machineProcessor = new AtMachineProcessor(state, Pc365.getPropertyService().getBoolean(Props.ENABLE_AT_DEBUG_LOG));
        ...
        while (payload <= freePayload - costOfOneAT && keys.hasNext()) {
            ...
            if (atAccountBalance >= AtConstants.getInstance().stepFee(at.getCreationBlockHeight())
                ...
                try {
                    ...
                    if (!Pc365.getFluxCapacitor().getValue(FluxValues.AT_FIX_BLOCK_4, blockHeight)) {
                        totalAmount = amount;
                    } else {
                        totalAmount += amount;
                    }
    ...
    private static long makeTransactions(AT at) throws AtException {
        long totalAmount = 0;
        if (!Pc365.getFluxCapacitor().getValue(FluxValues.AT_FIX_BLOCK_4, at.getHeight())) {
