...
public class AtApiPlatformImpl extends AtApiImpl {
    ...
    private static Long findTransaction(int startHeight, int endHeight, Long atID, int numOfTx, long minAmount) {
        return Pc365.getStores().getAtStore().findTransaction(startHeight, endHeight, atID, numOfTx, minAmount);
    }

    private static int findTransactionHeight(Long transactionId, int height, Long atID, long minAmount) {
        return Pc365.getStores().getAtStore().findTransactionHeight(transactionId, height, atID, minAmount);
    }
    ...
    public void putLastBlockHashInA(AtMachineState state) {
        ...
        b.put(Pc365.getBlockchain().getBlockAtHeight(state.getHeight() - 1).getBlockHash());
    ...
    public long getTypeForTxInA(AtMachineState state) {
        ...
        Transaction tx = Pc365.getBlockchain().getTransaction(txid);
    ...
    public long getAmountForTxInA(AtMachineState state) {
        ...
        if ((tx.getMessage() == null || Pc365.getFluxCapacitor().getValue(FluxValues.AT_FIX_BLOCK_2, state.getHeight())) && state.minActivationAmount() <= tx.getAmountNQT()) {
    ...
    public long getRandomIdForTxInA(AtMachineState state) {
        ...
        bf.put(Pc365.getBlockchain().getBlockAtHeight(blockHeight - 1).getGenerationSignature());
        bf.putLong(tx.getId());
    ...
    public void putLastBlockGenerationSignatureInA(AtMachineState state) {
        ...
        b.put(Pc365.getBlockchain().getBlockAtHeight(state.getHeight() - 1).getGenerationSignature());
    ...
    public long getCurrentBalance(AtMachineState state) {
        if (!Pc365.getFluxCapacitor().getValue(FluxValues.AT_FIX_BLOCK_2, state.getHeight())) {
            return 0;
        }
