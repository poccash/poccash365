...
public class BlockServiceImpl implements BlockService {
  ...
  public boolean verifyBlockSignature(Block block) throws BlockchainProcessor.BlockOutOfOrderException {
    try {
      ...
      if (genAccount == null || rewardAssignment == null || !Pc365.getFluxCapacitor().getValue(FluxValues.REWARD_RECIPIENT_ENABLE)) {
  ...
  public void apply(Block block) {
    ...
    if (!Pc365.getFluxCapacitor().getValue(FluxValues.REWARD_RECIPIENT_ENABLE)) {
  ...
  public long getBlockReward(Block block) {
    ...
    return BigInteger.valueOf(10000).multiply(BigInteger.valueOf(95).pow(month))
        .divide(BigInteger.valueOf(100).pow(month)).longValue() * Constants.ONE_PC365;
  }
  ...
  public void calculateBaseTarget(Block block, Block previousBlock) throws BlockOutOfOrderException {
    ...
    } else if (block.getHeight() < Constants.PC365_DIFF_ADJUST_CHANGE_BLOCK) {
