...
import core.*;
import core.Escrow.Decision;
import core.Escrow.DecisionType;
import core.db.Pc365Key;
import core.db.Pc365Key.LongKeyFactory;
import core.db.VersionedEntityTable;
import core.db.sql.DbKey.LinkKeyFactory;
...
public class EscrowServiceImpl implements EscrowService {
  ...
  public boolean isEnabled() {
    if(blockchain.getLastBlock().getHeight() >= Constants.PC365_ESCROW_START_BLOCK) {
      return true;
    }
  ...
  public void addEscrowTransaction(Account sender, Account recipient, Long id, Long amountNQT, int requiredSigners, Collection<Long> signers, int deadline, DecisionType deadlineAction) {
    final Pc365Key dbKey = escrowDbKeyFactory.newKey(id);
    ...
    Pc365Key senderDbKey = decisionDbKeyFactory.newKey(id, sender.getId());
    ...
    Pc365Key recipientDbKey = decisionDbKeyFactory.newKey(id, recipient.getId());
    ...
    for(Long signer : signers) {
      Pc365Key signerDbKey = decisionDbKeyFactory.newKey(id, signer);
  ...
  public DecisionType checkComplete(Escrow escrow) {
    ...
    for (Decision decision : Pc365.getStores().getEscrowStore().getDecisions(escrow.getId())) {
  ...
  public void updateOnBlock(Block block, int blockchainHeight) {
    ...
    if (!updatedEscrowIds.isEmpty()) {
      ...
      if (!resultTransactions.isEmpty()) {
        Pc365.getDbs().getTransactionDb().saveTransactions( resultTransactions);
      }
  ...
  public void saveResultTransaction(Block block, Long escrowId, Long recipientId, Long amountNQT, DecisionType decision, int blockchainHeight) {
    ...
    if(!Pc365.getDbs().getTransactionDb().hasTransaction(transaction.getId())) {
