...
import core.Attachment.AbstractAttachment;
import core.Attachment.AutomatedTransactionsCreation;
import core.Pc365Exception.NotValidException;
import core.Pc365Exception.ValidationException;
import core.assetexchange.AssetExchange;
import core.at.AT;
import core.at.AtConstants;
import core.at.AtController;
import core.at.AtException;
import core.fluxcapacitor.FluxCapacitor;
import core.fluxcapacitor.FluxValues;
import core.services.*;
import core.transactionduplicates.TransactionDuplicationKey;
...
import static core.Constants.FEE_QUANT;
import static core.Constants.ONE_PC365;
...
public abstract class TransactionType {
  ...
  private static final byte TYPE_PC365_MINING = 20; // jump some for easier nxt updating
  ...
  private static final byte SUBTYPE_PC365_MINING_REWARD_RECIPIENT_ASSIGNMENT = 0;
  ...
  public static void init(Blockchain blockchain, FluxCapacitor fluxCapacitor,
    ...
    Map<Byte, TransactionType> pc365MiningTypes = new HashMap<>();
    pc365MiningTypes.put(SUBTYPE_PC365_MINING_REWARD_RECIPIENT_ASSIGNMENT, Pc365Mining.REWARD_RECIPIENT_ASSIGNMENT);
    ...
    TRANSACTION_TYPES.put(TYPE_PC365_MINING, Collections.unmodifiableMap(pc365MiningTypes));
  ...
  public abstract Attachment.AbstractAttachment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws Pc365Exception.NotValidException;

  abstract Attachment.AbstractAttachment parseAttachment(JsonObject attachmentData) throws Pc365Exception.NotValidException;

  abstract void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException;
  ...
  public void parseAppendices(Transaction.Builder builder, int flags, byte version, ByteBuffer buffer) throws Pc365Exception.ValidationException {
    int position = 1;
    if ((flags & position) != 0) {
  ...
  public abstract static class Payment extends TransactionType {
    ...
    public static final TransactionType ORDINARY = new Payment() {
      ...
      @Override
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        if (transaction.getAmountNQT() <= 0 || transaction.getAmountNQT() >= Constants.MAX_BALANCE_NQT) {
          throw new Pc365Exception.NotValidException("Invalid ordinary payment");
        }
      }
    ...
    public static final TransactionType MULTI_OUT = new Payment() {
      ...
      @Override
      public Attachment.PaymentMultiOutCreation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws Pc365Exception.NotValidException {
        return new Attachment.PaymentMultiOutCreation(buffer, transactionVersion);
      }

      @Override
      Attachment.PaymentMultiOutCreation parseAttachment(JsonObject attachmentData) throws Pc365Exception.NotValidException {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        if (!fluxCapacitor.getValue(FluxValues.PRE_DYMAXION, transaction.getHeight())) {
          throw new Pc365Exception.NotCurrentlyValidException("Multi Out Payments are not allowed before the Pre Dymaxion block");
        }
        ...
        if (amountNQT <= 0
          ...
          throw new Pc365Exception.NotValidException("Invalid multi out payment");
        }
      }
    ...
    public static final TransactionType MULTI_SAME_OUT = new Payment() {
      ...
      @Override
      public Attachment.PaymentMultiSameOutCreation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws Pc365Exception.NotValidException {
        return new Attachment.PaymentMultiSameOutCreation(buffer, transactionVersion);
      }

      @Override
      Attachment.PaymentMultiSameOutCreation parseAttachment(JsonObject attachmentData) throws Pc365Exception.NotValidException {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        if (!fluxCapacitor.getValue(FluxValues.PRE_DYMAXION, transaction.getHeight())) {
          throw new Pc365Exception.NotCurrentlyValidException("Multi Same Out Payments are not allowed before the Pre Dymaxion block");
        }
  ...
  public abstract static class Messaging extends TransactionType {
    ...
    public static final TransactionType ARBITRARY_MESSAGE = new Messaging() {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (transaction.getAmountNQT() != 0) {
          throw new Pc365Exception.NotValidException("Invalid arbitrary message: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (! fluxCapacitor.getValue(FluxValues.DIGITAL_GOODS_STORE) && transaction.getMessage() == null) {
          throw new Pc365Exception.NotCurrentlyValidException("Missing message appendix not allowed before DGS block");
        }
      }
    ...
    public static final TransactionType ALIAS_ASSIGNMENT = new Messaging() {
      ...
      @Override
      public Attachment.MessagingAliasAssignment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws Pc365Exception.NotValidException {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (attachment.getAliasName().isEmpty()
          ...
          throw new Pc365Exception.NotValidException("Invalid alias assignment: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (!TextUtils.isInAlphabet(attachment.getAliasName())) {
          throw new Pc365Exception.NotValidException("Invalid alias name: " + attachment.getAliasName());
        }
        Alias alias = aliasService.getAlias(attachment.getAliasName());
        if (alias != null && alias.getAccountId() != transaction.getSenderId()) {
          throw new Pc365Exception.NotCurrentlyValidException("Alias already owned by another account: " + attachment.getAliasName());
        }
      }
    ...
    public static final TransactionType ALIAS_SELL = new Messaging() {
      ...
      @Override
      public Attachment.MessagingAliasSell parseAttachment(ByteBuffer buffer, byte transactionVersion) throws Pc365Exception.NotValidException {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        if (! fluxCapacitor.getValue(FluxValues.DIGITAL_GOODS_STORE, blockchain.getLastBlock().getHeight())) {
          throw new Pc365Exception.NotYetEnabledException("Alias transfer not yet enabled at height " + blockchain.getLastBlock().getHeight());
        }
        if (transaction.getAmountNQT() != 0) {
          throw new Pc365Exception.NotValidException("Invalid sell alias transaction: " + JSON.toJsonString(transaction.getJsonObject()));
        }
        final Attachment.MessagingAliasSell attachment =
                (Attachment.MessagingAliasSell) transaction.getAttachment();
        final String aliasName = attachment.getAliasName();
        if (aliasName == null || aliasName.isEmpty()) {
          throw new Pc365Exception.NotValidException("Missing alias name");
        }
        long priceNQT = attachment.getPriceNQT();
        if (priceNQT < 0 || priceNQT > Constants.MAX_BALANCE_NQT) {
          throw new Pc365Exception.NotValidException("Invalid alias sell price: " + priceNQT);
        }
        if (priceNQT == 0) {
          if (Genesis.CREATOR_ID == transaction.getRecipientId()) {
            throw new Pc365Exception.NotValidException("Transferring aliases to Genesis account not allowed");
          } else if (transaction.getRecipientId() == 0) {
            throw new Pc365Exception.NotValidException("Missing alias transfer recipient");
          }
        }
        final Alias alias = aliasService.getAlias(aliasName);
        if (alias == null) {
          throw new Pc365Exception.NotCurrentlyValidException("Alias hasn't been registered yet: " + aliasName);
        } else if (alias.getAccountId() != transaction.getSenderId()) {
          throw new Pc365Exception.NotCurrentlyValidException("Alias doesn't belong to sender: " + aliasName);
        }
      }
    ...
    public static final TransactionType ALIAS_BUY = new Messaging() {
      ...
      @Override
      public Attachment.MessagingAliasBuy parseAttachment(ByteBuffer buffer, byte transactionVersion) throws Pc365Exception.NotValidException {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (alias == null) {
          ...
          throw new Pc365Exception.NotCurrentlyValidException("Alias is owned by account other than recipient: "
                  + Convert.toUnsignedLong(alias.getAccountId()));
        }
        Alias.Offer offer = aliasService.getOffer(alias);
        if (offer == null) {
          throw new Pc365Exception.NotCurrentlyValidException("Alias is not for sale: " + aliasName);
        }
        if (transaction.getAmountNQT() < offer.getPriceNQT()) {
          ...
          throw new Pc365Exception.NotCurrentlyValidException(msg);
        }
        if (offer.getBuyerId() != 0 && offer.getBuyerId() != transaction.getSenderId()) {
          throw new Pc365Exception.NotCurrentlyValidException("Wrong buyer for " + aliasName + ": "
    ...
    public static final Messaging ACCOUNT_INFO = new Messaging() {
      ...
      @Override
      public Attachment.MessagingAccountInfo parseAttachment(ByteBuffer buffer, byte transactionVersion) throws Pc365Exception.NotValidException {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (Convert.toBytes(attachment.getName()).length > Constants.MAX_ACCOUNT_NAME_LENGTH
          ...
          throw new Pc365Exception.NotValidException("Invalid account info issuance: " + JSON.toJsonString(attachment.getJsonObject()));
        }
      }
  ...
  public abstract static class ColoredCoins extends TransactionType {
    ...
    public static final TransactionType ASSET_ISSUANCE = new ColoredCoins() {
      ...
      @Override
      public Attachment.ColoredCoinsAssetIssuance parseAttachment(ByteBuffer buffer, byte transactionVersion) throws Pc365Exception.NotValidException {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (attachment.getName().length() < Constants.MIN_ASSET_NAME_LENGTH
          ...
          throw new Pc365Exception.NotValidException("Invalid asset issuance: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (!TextUtils.isInAlphabet(attachment.getName())) {
          throw new Pc365Exception.NotValidException("Invalid asset name: " + attachment.getName());
        }
      }
    ...
    public static final TransactionType ASSET_TRANSFER = new ColoredCoins() {
      ...
      @Override
      public Attachment.ColoredCoinsAssetTransfer parseAttachment(ByteBuffer buffer, byte transactionVersion) throws Pc365Exception.NotValidException {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (transaction.getAmountNQT() != 0
          ...
          throw new Pc365Exception.NotValidException("Invalid asset transfer amount or comment: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (transaction.getVersion() > 0 && attachment.getComment() != null) {
          throw new Pc365Exception.NotValidException("Asset transfer comments no longer allowed, use message " +
        ...
        if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getQuantityQNT())) {
          throw new Pc365Exception.NotValidException("Invalid asset transfer asset or quantity: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (asset == null) {
          throw new Pc365Exception.NotCurrentlyValidException("Asset " + Convert.toUnsignedLong(attachment.getAssetId()) +
                  " does not exist yet");
        }
      }
    ...
    abstract static class ColoredCoinsOrderPlacement extends ColoredCoins {
      ...
      @Override
      final void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        Attachment.ColoredCoinsOrderPlacement attachment = (Attachment.ColoredCoinsOrderPlacement)transaction.getAttachment();
        if (attachment.getPriceNQT() <= 0 || attachment.getPriceNQT() > Constants.MAX_BALANCE_NQT
                || attachment.getAssetId() == 0) {
          throw new Pc365Exception.NotValidException("Invalid asset order placement: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        Asset asset = assetExchange.getAsset(attachment.getAssetId());
        if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getQuantityQNT())) {
          throw new Pc365Exception.NotValidException("Invalid asset order placement asset or quantity: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (asset == null) {
          throw new Pc365Exception.NotCurrentlyValidException("Asset " + Convert.toUnsignedLong(attachment.getAssetId()) +
                  " does not exist yet");
    ...
    public static final TransactionType ASK_ORDER_CANCELLATION = new ColoredCoinsOrderCancellation() {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (ask == null) {
          throw new Pc365Exception.NotCurrentlyValidException("Invalid ask order: " + Convert.toUnsignedLong(attachment.getOrderId()));
        }
        if (ask.getAccountId() != transaction.getSenderId()) {
          throw new Pc365Exception.NotValidException("Order " + Convert.toUnsignedLong(attachment.getOrderId()) + " was created by account "
    ...
    public static final TransactionType BID_ORDER_CANCELLATION = new ColoredCoinsOrderCancellation() {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (bid == null) {
          throw new Pc365Exception.NotCurrentlyValidException("Invalid bid order: " + Convert.toUnsignedLong(attachment.getOrderId()));
        }
  ...
  public abstract static class DigitalGoods extends TransactionType {
    ...
    final void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
      if (! fluxCapacitor.getValue(FluxValues.DIGITAL_GOODS_STORE, blockchain.getLastBlock().getHeight())) {
        throw new Pc365Exception.NotYetEnabledException("Digital goods listing not yet enabled at height " + blockchain.getLastBlock().getHeight());
      }
      if (transaction.getAmountNQT() != 0) {
        throw new Pc365Exception.NotValidException("Invalid digital goods transaction");
      }
      doValidateAttachment(transaction);
    }

    abstract void doValidateAttachment(Transaction transaction) throws Pc365Exception.ValidationException;
    ...
    public static final TransactionType LISTING = new DigitalGoods() {
      ...
      @Override
      public Attachment.DigitalGoodsListing parseAttachment(ByteBuffer buffer, byte transactionVersion) throws Pc365Exception.NotValidException {
      ...
      @Override
      void doValidateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        Attachment.DigitalGoodsListing attachment = (Attachment.DigitalGoodsListing) transaction.getAttachment();
        if (attachment.getName().isEmpty()
          ...
          throw new Pc365Exception.NotValidException("Invalid digital goods listing: " + JSON.toJsonString(attachment.getJsonObject()));
        }
      }
    ...
    public static final TransactionType DELISTING = new DigitalGoods() {
      ...
      void doValidateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (goods != null && transaction.getSenderId() != goods.getSellerId()) {
          throw new Pc365Exception.NotValidException("Invalid digital goods delisting - seller is different: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (goods == null || goods.isDelisted()) {
          throw new Pc365Exception.NotCurrentlyValidException("Goods " + Convert.toUnsignedLong(attachment.getGoodsId()) +
    ...
    public static final TransactionType PRICE_CHANGE = new DigitalGoods() {
      ...
      void doValidateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (attachment.getPriceNQT() <= 0 || attachment.getPriceNQT() > Constants.MAX_BALANCE_NQT
          ...
          throw new Pc365Exception.NotValidException("Invalid digital goods price change: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (goods == null || goods.isDelisted()) {
          throw new Pc365Exception.NotCurrentlyValidException("Goods " + Convert.toUnsignedLong(attachment.getGoodsId()) +
                  "not yet listed or already delisted");
    ...
    public static final TransactionType QUANTITY_CHANGE = new DigitalGoods() {
      ...
      void doValidateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (attachment.getDeltaQuantity() < -Constants.MAX_DGS_LISTING_QUANTITY
                ...
                || (goods != null && transaction.getSenderId() != goods.getSellerId())) {
          throw new Pc365Exception.NotValidException("Invalid digital goods quantity change: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (goods == null || goods.isDelisted()) {
          throw new Pc365Exception.NotCurrentlyValidException("Goods " + Convert.toUnsignedLong(attachment.getGoodsId()) +
                  "not yet listed or already delisted");
    ...
    public static final TransactionType PURCHASE = new DigitalGoods() {
      ...
      void doValidateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (attachment.getQuantity() <= 0 || attachment.getQuantity() > Constants.MAX_DGS_LISTING_QUANTITY
          ...
          throw new Pc365Exception.NotValidException("Invalid digital goods purchase: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
          throw new Pc365Exception.NotValidException("Only text encrypted messages allowed");
        }
        if (goods == null || goods.isDelisted()) {
          throw new Pc365Exception.NotCurrentlyValidException("Goods " + Convert.toUnsignedLong(attachment.getGoodsId()) +
                  "not yet listed or already delisted");
        ...
        if (attachment.getQuantity() > goods.getQuantity() || attachment.getPriceNQT() != goods.getPriceNQT()) {
          throw new Pc365Exception.NotCurrentlyValidException("Goods price or quantity changed: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (attachment.getDeliveryDeadlineTimestamp() <= blockchain.getLastBlock().getTimestamp()) {
          throw new Pc365Exception.NotCurrentlyValidException("Delivery deadline has already expired: " + attachment.getDeliveryDeadlineTimestamp());
        }
      }
    ...
    public static final TransactionType DELIVERY = new DigitalGoods() {
      ...
      @Override
      public Attachment.DigitalGoodsDelivery parseAttachment(ByteBuffer buffer, byte transactionVersion) throws Pc365Exception.NotValidException {
      ...
      void doValidateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (attachment.getGoods().getData().length > Constants.MAX_DGS_GOODS_LENGTH
          ...
          throw new Pc365Exception.NotValidException("Invalid digital goods delivery: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (purchase == null || purchase.getEncryptedGoods() != null) {
          throw new Pc365Exception.NotCurrentlyValidException("Purchase does not exist yet, or already delivered: " + JSON.toJsonString(attachment.getJsonObject()));
        }
      }
    ...
    public static final TransactionType FEEDBACK = new DigitalGoods() {
      ...
      void doValidateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (purchase != null &&
          ...
          throw new Pc365Exception.NotValidException("Invalid digital goods feedback: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (transaction.getEncryptedMessage() == null && transaction.getMessage() == null) {
          throw new Pc365Exception.NotValidException("Missing feedback message");
        }
        if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
          throw new Pc365Exception.NotValidException("Only text encrypted messages allowed");
        }
        if (transaction.getMessage() != null && ! transaction.getMessage().isText()) {
          throw new Pc365Exception.NotValidException("Only text public messages allowed");
        }
        if (purchase == null || purchase.getEncryptedGoods() == null) {
          throw new Pc365Exception.NotCurrentlyValidException("Purchase does not exist yet or not yet delivered");
        }
      }
    ...
    public static final TransactionType REFUND = new DigitalGoods() {
      ...
      void doValidateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (attachment.getRefundNQT() < 0 || attachment.getRefundNQT() > Constants.MAX_BALANCE_NQT
                ...
                (purchase.getBuyerId() != transaction.getRecipientId()
                        || transaction.getSenderId() != purchase.getSellerId()))) {
          throw new Pc365Exception.NotValidException("Invalid digital goods refund: " + JSON.toJsonString(attachment.getJsonObject()));
        }
        if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
          throw new Pc365Exception.NotValidException("Only text encrypted messages allowed");
        }
        if (purchase == null || purchase.getEncryptedGoods() == null || purchase.getRefundNQT() != 0) {
          throw new Pc365Exception.NotCurrentlyValidException("Purchase does not exist or is not delivered or is already refunded");
        }
      }
    ...
    public static final TransactionType EFFECTIVE_BALANCE_LEASING = new AccountControl() {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (transaction.getSenderId() == transaction.getRecipientId()
          ...
          throw new Pc365Exception.NotValidException("Invalid effective balance leasing: " + JSON.toJsonString(transaction.getJsonObject()) + " transaction " + transaction.getStringId());
        }
        if (recipientAccount == null
                || (recipientAccount.getPublicKey() == null && ! transaction.getStringId().equals("5081403377391821646"))) {
          throw new Pc365Exception.NotCurrentlyValidException("Invalid effective balance leasing: "
  ...
  public abstract static class Pc365Mining extends TransactionType {

    private Pc365Mining() {}

    @Override
    public final byte getType() {
      return TransactionType.TYPE_PC365_MINING;
    }
    ...
    public static final TransactionType REWARD_RECIPIENT_ASSIGNMENT = new Pc365Mining() {

      @Override
      public final byte getSubtype() {
        return TransactionType.SUBTYPE_PC365_MINING_REWARD_RECIPIENT_ASSIGNMENT;
      }
      ...
      @Override
      public Attachment.Pc365MiningRewardRecipientAssignment
      parseAttachment(ByteBuffer buffer, byte transactionVersion) {
        return new Attachment.Pc365MiningRewardRecipientAssignment(buffer, transactionVersion);
      }

      @Override
      Attachment.Pc365MiningRewardRecipientAssignment parseAttachment(JsonObject attachmentData) {
        return new Attachment.Pc365MiningRewardRecipientAssignment(attachmentData);
      }
      ...
      public TransactionDuplicationKey getDuplicationKey(Transaction transaction) {
        ...
        return new TransactionDuplicationKey(Pc365Mining.REWARD_RECIPIENT_ASSIGNMENT, Convert.toUnsignedLong(transaction.getSenderId()));
      }
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (sender == null) {
          throw new Pc365Exception.NotCurrentlyValidException("Sender not yet known ?!");
        }
        ...
        if (rewardAssignment != null && rewardAssignment.getFromHeight() >= height) {
          throw new Pc365Exception.NotCurrentlyValidException("Cannot reassign reward recipient before previous goes into effect: " + JSON.toJsonString(transaction.getJsonObject()));
        }
        Account recip = accountService.getAccount(transaction.getRecipientId());
        if (recip == null || recip.getPublicKey() == null) {
          throw new Pc365Exception.NotValidException("Reward recipient must have public key saved in blockchain: " + JSON.toJsonString(transaction.getJsonObject()));
        }

        if (fluxCapacitor.getValue(FluxValues.PRE_DYMAXION)) {
          if (transaction.getAmountNQT() != 0 || transaction.getFeeNQT() < FEE_QUANT) {
            throw new Pc365Exception.NotValidException("Reward recipient assignment transaction must have 0 send amount and at least minimum fee: " + JSON.toJsonString(transaction.getJsonObject()));
          }
        } else {
          if (transaction.getAmountNQT() != 0 || transaction.getFeeNQT() != Constants.ONE_PC365) {
            throw new Pc365Exception.NotValidException("Reward recipient assignment transaction must have 0 send amount and 1 fee: " + JSON.toJsonString(transaction.getJsonObject()));
          }
        }

        if (!Pc365.getFluxCapacitor().getValue(FluxValues.REWARD_RECIPIENT_ENABLE, height)) {
          throw new Pc365Exception.NotCurrentlyValidException("Reward recipient assignment not allowed before block " + Pc365.getFluxCapacitor().getStartingHeight(FluxValues.REWARD_RECIPIENT_ENABLE));
        }
      }
  ...
  public abstract static class AdvancedPayment extends TransactionType {
    ...
    public static final TransactionType ESCROW_CREATION = new AdvancedPayment() {
      ...
      @Override
      public Attachment.AdvancedPaymentEscrowCreation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws Pc365Exception.NotValidException {
        return new Attachment.AdvancedPaymentEscrowCreation(buffer, transactionVersion);
      }

      @Override
      Attachment.AdvancedPaymentEscrowCreation parseAttachment(JsonObject attachmentData) throws Pc365Exception.NotValidException {
      ...
      public Long calculateAttachmentTotalAmountNQT(Transaction transaction) {
        ...
        return Convert.safeAdd(attachment.getAmountNQT(), Convert.safeMultiply(attachment.getTotalSigners(), Constants.ONE_PC365));
      }
      ...
      final void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
        ...
        signers.forEach(signer -> accountService.addToBalanceAndUnconfirmedBalanceNQT(accountService.getOrAddAccount(signer), Constants.ONE_PC365));
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (transaction.getSenderId() == transaction.getRecipientId()) {
          throw new Pc365Exception.NotValidException("Escrow must have different sender and recipient");
        }
        totalAmountNQT = Convert.safeAdd(totalAmountNQT, attachment.getTotalSigners() * Constants.ONE_PC365);
        if (transaction.getAmountNQT() != 0) {
          throw new Pc365Exception.NotValidException("Transaction sent amount must be 0 for escrow");
        }
        if (totalAmountNQT.compareTo(0L) < 0 ||
                totalAmountNQT.compareTo(Constants.MAX_BALANCE_NQT) > 0)
        {
          throw new Pc365Exception.NotValidException("Invalid escrow creation amount");
        }
        if (transaction.getFeeNQT() < Constants.ONE_PC365) {
          throw new Pc365Exception.NotValidException("Escrow transaction must have a fee at least 1 pc365");
        }
        if (attachment.getRequiredSigners() < 1 || attachment.getRequiredSigners() > 10) {
          throw new Pc365Exception.NotValidException("Escrow required signers much be 1 - 10");
        }
        if (attachment.getRequiredSigners() > attachment.getTotalSigners()) {
          throw new Pc365Exception.NotValidException("Cannot have more required than signers on escrow");
        }
        if (attachment.getTotalSigners() < 1 || attachment.getTotalSigners() > 10) {
          throw new Pc365Exception.NotValidException("Escrow transaction requires 1 - 10 signers");
        }
        if (attachment.getDeadline() < 1 || attachment.getDeadline() > 7776000) { // max deadline 3 months
          throw new Pc365Exception.NotValidException("Escrow deadline must be 1 - 7776000 seconds");
        }
        if (attachment.getDeadlineAction() == null || attachment.getDeadlineAction() == Escrow.DecisionType.UNDECIDED) {
          throw new Pc365Exception.NotValidException("Invalid deadline action for escrow");
        }
        if (attachment.getSigners().contains(transaction.getSenderId()) ||
                attachment.getSigners().contains(transaction.getRecipientId())) {
          throw new Pc365Exception.NotValidException("Escrow sender and recipient cannot be signers");
        }
        if (!escrowService.isEnabled()) {
          throw new Pc365Exception.NotYetEnabledException("Escrow not yet enabled");
        }
      }
    ...
    public static final TransactionType ESCROW_SIGN = new AdvancedPayment() {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        Attachment.AdvancedPaymentEscrowSign attachment = (Attachment.AdvancedPaymentEscrowSign) transaction.getAttachment();
        if (transaction.getAmountNQT() != 0 || transaction.getFeeNQT() != Constants.ONE_PC365) {
          throw new Pc365Exception.NotValidException("Escrow signing must have amount 0 and fee of 1");
        }
        if (attachment.getEscrowId() == null || attachment.getDecision() == null) {
          throw new Pc365Exception.NotValidException("Escrow signing requires escrow id and decision set");
        }
        Escrow escrow = escrowService.getEscrowTransaction(attachment.getEscrowId());
        if (escrow == null) {
          throw new Pc365Exception.NotValidException("Escrow transaction not found");
        }
        if (!escrowService.isIdSigner(transaction.getSenderId(), escrow) &&
          ...
          throw new Pc365Exception.NotValidException("Sender is not a participant in specified escrow");
        }
        if (escrow.getSenderId().equals(transaction.getSenderId()) && attachment.getDecision() != Escrow.DecisionType.RELEASE) {
          throw new Pc365Exception.NotValidException("Escrow sender can only release");
        }
        if (escrow.getRecipientId().equals(transaction.getSenderId()) && attachment.getDecision() != Escrow.DecisionType.REFUND) {
          throw new Pc365Exception.NotValidException("Escrow recipient can only refund");
        }
        if (!escrowService.isEnabled()) {
          throw new Pc365Exception.NotYetEnabledException("Escrow not yet enabled");
        }
    ...
    public static final TransactionType ESCROW_RESULT = new AdvancedPayment() {
      ...
      @Override
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        throw new Pc365Exception.NotValidException("Escrow result never validates");
      }
    ...
    public static final TransactionType SUBSCRIPTION_SUBSCRIBE = new AdvancedPayment() {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (attachment.getFrequency() == null ||
                attachment.getFrequency() < Constants.PC365_SUBSCRIPTION_MIN_FREQ ||
                attachment.getFrequency() > Constants.PC365_SUBSCRIPTION_MAX_FREQ) {
          throw new Pc365Exception.NotValidException("Invalid subscription frequency");
        }
        if (transaction.getAmountNQT() < Constants.ONE_PC365 || transaction.getAmountNQT() > Constants.MAX_BALANCE_NQT) {
          throw new Pc365Exception.NotValidException("Subscriptions must be at least one pc365");
        }
        if (transaction.getSenderId() == transaction.getRecipientId()) {
          throw new Pc365Exception.NotValidException("Cannot create subscription to same address");
        }
        if (!subscriptionService.isEnabled()) {
          throw new Pc365Exception.NotYetEnabledException("Subscriptions not yet enabled");
        }
      }
    ...
    public static final TransactionType SUBSCRIPTION_CANCEL = new AdvancedPayment() {
      ...
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        ...
        if (attachment.getSubscriptionId() == null) {
          throw new Pc365Exception.NotValidException("Subscription cancel must include subscription id");
        }

        Subscription subscription = subscriptionService.getSubscription(attachment.getSubscriptionId());
        if (subscription == null) {
          throw new Pc365Exception.NotValidException("Subscription cancel must contain current subscription id");
        }

        if (!subscription.getSenderId().equals(transaction.getSenderId()) &&
                !subscription.getRecipientId().equals(transaction.getSenderId())) {
          throw new Pc365Exception.NotValidException("Subscription cancel can only be done by participants");
        }

        if (!subscriptionService.isEnabled()) {
          throw new Pc365Exception.NotYetEnabledException("Subscription cancel not yet enabled");
        }
      }
    ...
    public static final TransactionType SUBSCRIPTION_PAYMENT = new AdvancedPayment() {
      ...
      @Override
      void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        throw new Pc365Exception.NotValidException("Subscription payment never validates");
      }
  ...
  public abstract static class AutomatedTransactions extends TransactionType{
    ...
    final void validateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
      if (transaction.getAmountNQT() != 0) {
        throw new Pc365Exception.NotValidException("Invalid automated transaction transaction");
      }
      doValidateAttachment(transaction);
    }
    ...
    public static final TransactionType AUTOMATED_TRANSACTION_CREATION = new AutomatedTransactions(){
      ...
      void doValidateAttachment(Transaction transaction)
        ...
        if (! fluxCapacitor.getValue(FluxValues.AUTOMATED_TRANSACTION_BLOCK, blockchain.getLastBlock().getHeight())) {
          throw new Pc365Exception.NotYetEnabledException("Automated Transactions not yet enabled at height " + blockchain.getLastBlock().getHeight());
        }
        if (transaction.getSignature() != null && accountService.getAccount(transaction.getId()) != null) {
          ...
          if (existingAccount.getPublicKey() != null && !Arrays.equals(existingAccount.getPublicKey(), new byte[32]))
            throw new Pc365Exception.NotValidException("Account with id already exists");
        }
        ...
        try {
          ...
          throw new Pc365Exception.NotCurrentlyValidException("Invalid AT creation bytes", e);
        }
        ...
        if (transaction.getFeeNQT() <  requiredFee){
          throw new Pc365Exception.NotValidException("Insufficient fee for AT creation. Minimum: " + Convert.toUnsignedLong(requiredFee / Constants.ONE_PC365));
        }
        if (fluxCapacitor.getValue(FluxValues.AT_FIX_BLOCK_3)) {
          if (attachment.getName().length() > Constants.MAX_AUTOMATED_TRANSACTION_NAME_LENGTH) {
            throw new Pc365Exception.NotValidException("Name of automated transaction over size limit");
          }
          if (attachment.getDescription().length() > Constants.MAX_AUTOMATED_TRANSACTION_DESCRIPTION_LENGTH) {
            throw new Pc365Exception.NotValidException("Description of automated transaction over size limit");
          }
        }
      }
    ...
    public static final TransactionType AT_PAYMENT = new AutomatedTransactions() {
      ...
      @Override
      void doValidateAttachment(Transaction transaction) throws Pc365Exception.ValidationException {
        throw new Pc365Exception.NotValidException("AT payment never validates");
      }
  ...
  protected Fee getBaselineFee(int height) {
    return new Fee((fluxCapacitor.getValue(FluxValues.PRE_DYMAXION, height) ? FEE_QUANT : ONE_PC365), 0);
  }
