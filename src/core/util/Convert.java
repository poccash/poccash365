...
public final class Convert {
  ...
  private static final Pc365Crypto pc365Crypto = Pc365Crypto.getInstance();
  ...
  public static long parseAccountId(String account) {
    Pc365Address address = Pc365Address.fromEither(account);
    return address == null ? 0 : address.getPc365ID().getSignedLongId();
  }

  public static String rsAccount(long accountId) {
    return "PC365-" + Crypto.rsEncode(accountId);
  }

  public static long fullHashToId(byte[] hash) {
    return pc365Crypto.hashToId(hash).getSignedLongId();
  }
  ...
  public static String readString(ByteBuffer buffer, int numBytes, int maxLength) throws Pc365Exception.NotValidException {
    if (numBytes > 3 * maxLength) {
      throw new Pc365Exception.NotValidException("Max parameter length exceeded");
    }
  ...
  public static long parseNXT(String nxt) {
    return parseStringFraction(nxt, 8, Constants.MAX_BALANCE_PC365);
  }
