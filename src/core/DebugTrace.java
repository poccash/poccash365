...
public final class DebugTrace {
  ...
  static void init(PropertyService propertyService, BlockchainProcessor blockchainProcessor,
    ...
    QUOTE           = propertyService.getString(Props.CORE_DEBUG_TRACE_QUOTE);
    SEPARATOR       = propertyService.getString(Props.CORE_DEBUG_TRACE_SEPARATOR);
    LOG_UNCONFIRMED = propertyService.getBoolean(Props.CORE_DEBUG_LOG_CONFIRMED);

    DebugTrace.assetExchange = assetExchange;
    DebugTrace.dgsGoodsStoreService = dgsGoodsStoreService;

    List<String> accountIdStrings = propertyService.getStringList(Props.CORE_DEBUG_TRACE_ACCOUNTS);
    String logName = propertyService.getString(Props.CORE_DEBUG_TRACE_LOG);
  ...
  private Map<String,String> lessorGuaranteedBalance(Account account, long lesseeId) {
    ...
    map.put("timestamp", String.valueOf(Pc365.getBlockchain().getLastBlock().getTimestamp()));
    map.put("height", String.valueOf(Pc365.getBlockchain().getHeight()));
