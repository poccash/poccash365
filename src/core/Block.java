...
public class Block {
  ...
  Block(int version, int timestamp, long previousBlockId, long totalAmountNQT, long totalFeeNQT,
      ...
      long nonce, byte[] blockATs, int height) throws Pc365Exception.ValidationException {

    if (payloadLength > Pc365.getFluxCapacitor().getValue(FluxValues.MAX_PAYLOAD_LENGTH, height) || payloadLength < 0) {
      throw new Pc365Exception.NotValidException(
    ...
    if (transactions != null) {
      ...
      if (blockTransactions.get().size() > (Pc365.getFluxCapacitor().getValue(FluxValues.MAX_NUMBER_TRANSACTIONS, height))) {
        throw new Pc365Exception.NotValidException(
      ...
      for (Transaction transaction : this.blockTransactions.get()) {
        if (transaction.getId() <= previousId && previousId != 0) {
          throw new Pc365Exception.NotValidException("Block transactions are not sorted!");
        }
  ...
  public Block(int version, int timestamp, long previousBlockId, long totalAmountNQT, long totalFeeNQT, int payloadLength, byte[] payloadHash, byte[] generatorPublicKey, byte[] generationSignature, byte[] blockSignature, byte[] previousBlockHash, BigInteger cumulativeDifficulty, long baseTarget,
      long nextBlockId, int height, Long id, long nonce, byte[] blockATs) throws Pc365Exception.ValidationException {
  ...
  private TransactionDb transactionDb() {
    return Pc365.getDbs().getTransactionDb();
  }
  ...
  static Block parseBlock(JsonObject blockData, int height) throws Pc365Exception.ValidationException {
    try {
      ...
      for (JsonElement transactionData : transactionsData) {
        ...
        if (transaction.getSignature() != null && blockTransactions.put(transaction.getId(), transaction) != null) {
          throw new Pc365Exception.NotValidException("Block contains duplicate transactions: " + transaction.getStringId());
        }
      }
  ...
  public byte[] getBytes() {
    ...
    if (version < 3) {
      buffer.putInt((int) (totalAmountNQT / Constants.ONE_PC365));
      buffer.putInt((int) (totalFeeNQT / Constants.ONE_PC365));
    } else {
